...
class CliTelemetryClient(object):
    ...
    def flush(self, force=False):
        if force or self._counter >= self._batch:
            self._logger.info('Accumulated {} events. Flush the clients.'.format(self._counter))
            for instrumentation_key, client in self._clients.items():
                self._logger.info('Flush client {}.'.format(instrumentation_key))
                client.flush()
            self._counter = 0
    ...
    def _parse_in_json(self, data):
        try:
            ...
            self._logger.debug('Fail to parse data {} to JSON. Reason {}.'.format(data, err))
        except Exception as err:
            self._logger.debug('Fail to parse data {} to JSON. Unexpected reason {}.'.format(data, err))
...
class _NoRetrySender(SynchronousSender):
    ...
    def send(self, data_to_send):
        ...
        try:
            ...
            self._logger.error('Upload failed. HTTPError: {}'.format(e))
        except OSError as e:  # socket timeout
            # stop retry during socket timeout
            self._logger.error('Upload failed. OSError: {}'.format(e))
        except Exception as e:  # pylint: disable=broad-except
            self._logger.error('Unexpected exception: {}'.format(e))
        finally:
            self._logger.info('Finish sending {} bytes in Duration {} s'.format(
                len(content),
