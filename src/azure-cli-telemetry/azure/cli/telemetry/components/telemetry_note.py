...
class TelemetryNote(portalocker.utils.Lock):
    TELEMETRY_NOTE_FILENAME = 'telemetry.txt'
    ...
    def get_file_path(config_dir):
        return os.path.join(config_dir, TelemetryNote.TELEMETRY_NOTE_FILENAME)
    ...
    def get_last_sent(self):
        ...
        try:
            ...
            self._logger.info("Read timestamp from the note. The last send was {}".format(last_send))
        except (ValueError, IOError) as err:
            last_send = datetime.datetime.min
            self._logger.warning("Fail to parse or read the timestamp '{}' in the note file. "
                                 "Set the last send time to minimal. Reason: {}".format(raw, err))
    ...
    def update_telemetry_note(self, next_send):
        ...
        for retry in range(3):
            try:
                ...
                self._logger.warning('Fail to update the note file. This is the {} try.'.format(retry + 1))
        else:
            # TODO: how to save this?
            self._logger.error('Fail to update the note file after three retry.')

        self._logger.info('Update the last send in note to {}'.format(next_send))
        self.touch()
    ...
    def touch(self):
        ...
        self._logger.info('Update the note mtime to {}'.format(datetime.datetime.fromtimestamp(st_mtime)))
