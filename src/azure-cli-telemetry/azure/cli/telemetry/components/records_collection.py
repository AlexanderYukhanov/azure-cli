...
class RecordsCollection(object):
    ...
    def snapshot_and_read(self):
        ...
        self._logger.info('{} cache files to move.'.format(len(candidates)))
        self._logger.info('Create temp folder {}'.format(tmp))
        ...
        for each in candidates:
            if stat.S_ISREG(each[1].st_mode):
                try:
                    ...
                    self._logger.info('Move file {} to {}'.format(
                        os.path.join(folder, each[0]),
                        os.path.join(tmp, each[0])))
                except IOError as err:
                    self._logger.warning('Fail to move file from {} to {}. {}'.format(
        ...
        shutil.rmtree(tmp,
                      ignore_errors=True,
                      onerror=lambda _, p, tr: self._logger.error('Fail to remove file {}'.format(p)))
        self._logger.info('Remove directory {}'.format(tmp))
    ...
    def _read_file(self, path):
        ...
        try:
            with open(path, mode='r') as fh:
                ...
                self._logger.info("Processed file {} into {} records.".format(path, len(self._records)))
        except IOError as err:
            self._logger.warning("Fail to open file {}. Reason: {}".format(path, err))
    ...
    def _add_record(self, content_line):
        ...
        try:
            ...
            self._logger.warning("Fail to parse a line of the record {}. Error {}.".format(content_line, err))
