...
def should_upload(config_dir):
    ...
    Before the client request a telemetry add process, it can invoke this methods to test if it is the right moment.
    The conditions are:
    ...
    if not os.path.exists(telemetry_note_path):
        logger.info('Positive: The {} does not exist.'.format(telemetry_note_path))
        return True

    file_stat = os.stat(telemetry_note_path)
    if not stat.S_ISREG(file_stat.st_mode):
        logger.warning('Negative: The {} is not a regular file.'.format(telemetry_note_path))
        return False

    modify_time = datetime.fromtimestamp(file_stat.st_mtime)
    if datetime.now() - modify_time < MANDATORY_WAIT_PERIOD:
        logger.warning('Negative: The {} was modified at {}, which in less than {} s'.format(
...
def save_payload(config_dir, payload):
    ...
    if payload:
        ...
        if cache_saver:
            ...
            logger.info('Save telemetry record of length {} in cache'.format(len(payload)))
...
def _create_rotate_file_logger(log_dir):
    ...
    try:
        ...
        logging.getLogger('telemetry.save').warning(
            'Fail to create telemetry cache directory for {}. Reason {}.'.format(cache_name, err))
        return None
