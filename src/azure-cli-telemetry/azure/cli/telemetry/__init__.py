...
def save(config_dir, payload):
    from .util import save_payload, should_upload
    from azure.cli.telemetry.components.telemetry_logging import get_logger
...
def main():
    ...
    try:
        ...
        logger = get_logger('main')
        logger.info('Attempt start. Configuration directory [{}].'.format(sys.argv[1]))
        ...
        try:
            ...
            logger.info(
                'Lock out from note file under {} which means another process is running. Exit 0.'.format(config_dir))
            sys.exit(0)
        except IOError as err:
            logger.warning('Unexpected IO Error {}. Exit 1.'.format(err))
            sys.exit(1)
        except Exception as err:  # pylint: disable=broad-except
            logger.error('Unexpected Error {}. Exit 2.'.format(err))
